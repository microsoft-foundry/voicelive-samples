...
export class VoiceAssistant {
  ...
  async startConversation(): Promise<void> {
    ...
    try {
      ...
      // Let LLM speak first to initiate the conversation
      this.session?.sendEvent({
        type: "response.create",
  ...
  private async configureSession(config: VoiceAssistantConfig): Promise<void> {
    ...
    // Only register set_reference_text tool when Read Along scenario is selected
    if (config.paScenario === "readAlong") {
      sessionConfig.tools = [
        {
  ...
  private async sendAudioData(audioData: ArrayBuffer): Promise<void> {
    ...
    try {
      ...
      // Cache audio for pronunciation assessment
