# 🎉 **Voice Live Education Demo — Pronunciation Assessment (PA)**

A web-based English pronunciation coaching assistant built on the **Azure Voice Live SDK** for TypeScript and the **Azure Speech SDK** for pronunciation assessment. The assistant listens to the user, evaluates pronunciation in real time, and responds with targeted coaching feedback.
...
```bash
cd samples/voice-live-education-demo
npm install
```
...
4. Pick a **PA scenario** (Conversation / Concise / Read Along) — the instructions textarea is auto-filled with the matching coaching prompt and can be edited
5. Click **"Connect"**
...
Pronunciation Assessment is the core feature of this demo. It is powered by the Azure Speech SDK, runs alongside the Voice Live conversation, and is **always enabled** — the assistant uses the PA result to give targeted pronunciation feedback through the LLM.

### **Configuration**

1. Pick a **PA scenario** in the Configuration panel (see below).
2. The instructions textarea is auto-filled with the scenario's predefined coaching prompt; switching scenarios refreshes it. Feel free to edit before connecting.
3. (Optional) Toggle **"PA with reference text"** to switch between reference-text mode and streaming mode (disabled for Read Along — the reference text comes from the tool call).
...
- The Speech SDK `SpeechConfig` is initialized on connect and is used for every turn.
- The `set_reference_text` function tool is only registered with the session when the **Read Along** scenario is selected.
- PA results are sent back to the LLM as `additionalInstructions` on `response.create` so the assistant can ground its feedback on the per-word scores.
- Server VAD is tuned for PA (longer prefix padding and silence timeout) so utterances are captured cleanly.
